...
@section('header_title', 'Edit Discount')
    ...
    <div class="soma-page-inner max-w-5xl">
        ...
        <div class="flex items-center justify-between gap-4 mb-6">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h1 class="text-2xl font-bold text-slate-900 tracking-tight">Edit Discount</h1>
                    <p class="text-sm text-slate-500">Update promotional rules and parameters for "{{ $discount->name }}"</p>
                </div>
            </div>
        </div>
                    ...
                    <div class="soma-card p-6">
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <input type="text" name="name" id="discount-name" value="{{ old('name', $discount->name) }}" placeholder="e.g. Weekend Flash Sale, Ramadan Special" class="soma-input" required>
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    ...
                                    <input type="text" name="code" id="discount-code" value="{{ old('code', $discount->code) }}" placeholder="e.g. FLASH20, SAVE500 (Leave blank for automatic)" class="soma-input pl-9 uppercase tracking-wider font-mono">
                                </div>
                            ...
                            <div>
                                ...
                                <textarea name="description" id="discount-desc" rows="2" placeholder="Optional notes about terms, eligibility, or purpose..." class="soma-input">{{ old('description', $discount->description) }}</textarea>
                            </div>
                        </div>
                    </div>
                        ...
                        <div class="space-y-4">
                            <div>
                                ...
                                <div class="grid grid-cols-2 gap-3">
                                    <label class="relative flex items-center gap-3 p-3.5 rounded-xl border-2 cursor-pointer transition-all {{ old('type', $discount->type) === 'percentage' ? 'border-blue-600 bg-blue-50/40' : 'border-slate-200' }}" id="type-percentage-label">
                                        <input type="radio" name="type" value="percentage" {{ old('type', $discount->type) === 'percentage' ? 'checked' : '' }} onchange="updateType('percentage')" class="text-blue-600 focus:ring-blue-500">
                                        <div>
                                    ...
                                    <label class="relative flex items-center gap-3 p-3.5 rounded-xl border-2 cursor-pointer transition-all {{ old('type', $discount->type) === 'fixed' ? 'border-blue-600 bg-blue-50/40' : 'border-slate-200' }}" id="type-fixed-label">
                                        <input type="radio" name="type" value="fixed" {{ old('type', $discount->type) === 'fixed' ? 'checked' : '' }} onchange="updateType('fixed')" class="text-blue-600 focus:ring-blue-500">
                                        <div>
                                            <div class="text-xs font-bold text-slate-900 flex items-center gap-1.5">
                                ...
                                <div>
                                    ...
                                    <div class="relative">
                                        <div class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-3 text-slate-400 font-medium text-xs" id="value-symbol">
                                            {{ old('type', $discount->type) === 'percentage' ? '%' : 'KES' }}
                                        </div>
                                        <input type="number" step="0.01" min="0.01" name="value" id="discount-value" value="{{ old('value', $discount->value) }}" placeholder="0.00" class="soma-input pl-9" required>
                                    </div>
                                    <p class="text-[11px] text-slate-400 mt-1" id="value-help">e.g. 15 for 15% discount</p>
                                </div>

                                <div id="max-discount-wrapper" style="{{ old('type', $discount->type) === 'fixed' ? 'display:none;' : '' }}">
                                    <label class="block text-xs font-semibold text-slate-700 mb-1">
                                    ...
                                    <div class="relative">
                                        ...
                                        <input type="number" step="0.01" min="0" name="max_discount" id="max-discount" value="{{ old('max_discount', $discount->max_discount) }}" placeholder="No limit" class="soma-input pl-12">
                                    </div>
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    ...
                                    <input type="number" step="0.01" min="0" name="min_spend" id="min-spend" value="{{ old('min_spend', $discount->min_spend) }}" placeholder="0.00 (No minimum)" class="soma-input pl-12">
                                </div>
                        ...
                        <div class="space-y-4">
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="number" min="1" step="1" name="usage_limit" id="usage-limit" value="{{ old('usage_limit', $discount->usage_limit) }}" placeholder="Unlimited" class="soma-input">
                                    <p class="text-[11px] text-slate-400 mt-1">Maximum times this discount can be used.</p>
                                </div>
                                <div>
                                    <label class="block text-xs font-semibold text-slate-700 mb-1">
                                        Current Usage Count
                                    </label>
                                    <input type="text" value="{{ $discount->used_count }} times redeemed" class="soma-input bg-slate-50 text-slate-500" readonly>
                                    <p class="text-[11px] text-slate-400 mt-1">Total redemptions to date.</p>
                                </div>
                            </div>
                            ...
                            <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="datetime-local" name="starts_at" id="starts-at" value="{{ old('starts_at', $discount->starts_at ? $discount->starts_at->format('Y-m-d\TH:i') : '') }}" class="soma-input">
                                ...
                                <div>
                                    ...
                                    <input type="datetime-local" name="ends_at" id="ends-at" value="{{ old('ends_at', $discount->ends_at ? $discount->ends_at->format('Y-m-d\TH:i') : '') }}" class="soma-input">
                ...
                <div class="space-y-6">
                    ...
                    <div class="soma-card p-5">
                        ...
                        <label class="flex items-center gap-3 cursor-pointer">
                            <input type="checkbox" name="is_active" value="1" {{ old('is_active', $discount->is_active) ? 'checked' : '' }} class="w-4 h-4 rounded text-blue-600 focus:ring-blue-500 border-slate-300">
                            <div>
                                <div class="text-xs font-semibold text-slate-900">Discount is Active</div>
                                <div class="text-[11px] text-slate-500">Uncheck to temporarily disable this discount</div>
                            </div>
                        </label>
                    </div>
                    ...
                    <div class="soma-card p-5 bg-gradient-to-br from-slate-900 to-slate-800 text-white rounded-2xl shadow-lg">
                        ...
                        <div class="space-y-3">
                            <div id="preview-badge" class="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full text-xs font-bold bg-blue-500 text-white shadow-sm">
                                <span id="preview-val">{{ $discount->formatted_value }}</span>
                            </div>
                            ...
                            <div class="bg-slate-800/80 rounded-xl p-3 border border-slate-700 space-y-1 text-xs">
                                <div class="flex justify-between text-slate-300">
                                    <span>Code:</span>
                                    <span class="font-mono font-bold text-amber-400" id="preview-code">{{ $discount->code ?: 'AUTOMATIC' }}</span>
                                </div>
                                <div class="flex justify-between text-slate-300">
                                    <span>Min Spend:</span>
                                    <span id="preview-min">{{ $discount->min_spend ? 'KES ' . number_format($discount->min_spend, 2) : 'None' }}</span>
                                </div>
                                <div class="flex justify-between text-slate-300">
                                    <span>Max Cap:</span>
                                    <span id="preview-cap">{{ $discount->max_discount ? 'KES ' . number_format($discount->max_discount, 2) : 'None' }}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="soma-card p-5 space-y-3">
                        <button type="submit" class="soma-btn-primary w-full justify-center py-2.5 font-semibold">
                            <i class="fas fa-check text-xs"></i> Update Discount
                        </button>
...
@push('scripts')
<script>
let currentType = '{{ old('type', $discount->type) }}';
